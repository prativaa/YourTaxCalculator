class TaxCalculationService
	...
	def calculate_tax
  	...
  	puts "Base taxable income: #{@base_taxable_income}"
		...
		if(@taxable_income <= @base_taxable_income )
			...
			remaining_taxable_money, tax = calculate_tax_band(10.0, tax, 200000, remaining_taxable_money, taxes)
			return tax, taxes if remaining_taxable_money.zero?

			remaining_taxable_money, tax = calculate_tax_band(20.0, tax, 300000, remaining_taxable_money, taxes)
			return tax, taxes if remaining_taxable_money.zero?

			remaining_taxable_money, tax = calculate_tax_band(30.0, tax, @marital_status=="unmarried" ? 1000000 : 900000, remaining_taxable_money, taxes)
			return tax, taxes if remaining_taxable_money.zero?

			remaining_taxable_money, tax = calculate_tax_band(36.0, tax, 3000000, remaining_taxable_money, taxes)
			return tax, taxes if remaining_taxable_money.zero?

			remaining_taxable_money, tax = calculate_tax_band(39.0, tax, remaining_taxable_money, remaining_taxable_money, taxes)
			return tax, taxes if remaining_taxable_money.zero?
	...
	def calculate_base_tax(taxes, tax_rate, taxable_income)
		calculated_rate = @ssf_amt>0 ? 0 : (tax_rate/100)
